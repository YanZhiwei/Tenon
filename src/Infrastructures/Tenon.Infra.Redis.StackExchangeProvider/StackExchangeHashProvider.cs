...
public partial class StackExchangeProvider
{
    ...
    public IEnumerable<string> HKeys(string cacheKey)
    {
        ...
        return Enumerable.Select<RedisValue, string>(keys, x => x.ToString()).ToList();
    }
    ...
    {
        return Enumerable.Select<RedisValue, string>(_redisDatabase.HashValues(cacheKey), x => x.ToString()).ToList();
    }
    ...
    public IDictionary<string, string> HMGet(string cacheKey, IEnumerable<string> fields)
    {
        ...
        for (var i = 0; i < fields.Count(); i++)
        {
            ...
            if (!dict.ContainsKey(key))
                dict.Add(key, Enumerable.ElementAt<RedisValue>(list, i));
        }
    ...
    {
        return Enumerable.Select<RedisValue, string>((await _redisDatabase.HashValuesAsync(cacheKey)), x => x.ToString()).ToList();
    }
    ...
    public async Task<IDictionary<string, string>> HMGetAsync(string cacheKey, IEnumerable<string> fields)
    {
        ...
        for (var i = 0; i < fields.Count(); i++)
        {
            ...
            if (!dict.ContainsKey(key))
            {
                dict.Add(key, Enumerable.ElementAt<RedisValue>(res, i));
            }
        }
